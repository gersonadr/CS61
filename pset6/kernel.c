...
int find_free_page_and_allocates (int8_t pid);
int find_free_page ();
x86_pagetable* copy_pagetable(x86_pagetable * reference_pagetable, int8_t pid);
...
void process_setup(pid_t pid, int program_number) {
    ...
    // Copies kernel_pagetable as processes's page table
    // (kernel data+code will be unacessible by process, and kernel can quickly access its data without changing pagetables)
    processes[pid].p_pagetable = copy_pagetable(kernel_pagetable, pid);
    int r = program_load(&processes[pid], program_number);
    assert(r >= 0);
    // Stack pointer starts at the last virtual memory position 
    processes[pid].p_registers.reg_esp = MEMSIZE_VIRTUAL;
    uintptr_t stack_page = processes[pid].p_registers.reg_esp - PAGESIZE;
    // Finds and allocates a PA
    intptr_t p_addr = find_free_page_and_allocates(pid);
    // Maps the pagetable at free PA found previously
    virtual_memory_map(processes[pid].p_pagetable, stack_page, p_addr << PAGESHIFT,
                       PAGESIZE, PTE_P|PTE_W|PTE_U);
    processes[pid].p_state = P_RUNNABLE;
}

x86_pagetable* copy_pagetable(x86_pagetable* reference, int8_t pid){

    int level1_page = find_free_page_and_allocates(pid);    
    if(level1_page < 0)
      return NULL;

    int level2_page = find_free_page_and_allocates(pid);    
    if(level2_page < 0)
      return NULL;

    x86_pagetable *level1 = (x86_pagetable*) (level1_page<<PAGESHIFT);
    x86_pagetable *level2 = (x86_pagetable*) (level2_page<<PAGESHIFT);
  
    memset(level1, 0, PAGESIZE);

    //Copy reference page table on new Level 2 page
    memcpy(level2, reference, PAGESIZE);

    level1->entry[0] = (x86_pageentry_t) level2_page<<PAGESHIFT | PTE_P | PTE_W| PTE_U;

    // User process should not access kernel memory
    virtual_memory_map(level1, 0, 0, PROC_START_ADDR, PTE_P|PTE_W);
...
void exception(x86_registers* reg) {
    ...
    case INT_SYS_PAGE_ALLOC: {
        ...
        // Finds empty physical page
        int free_page = find_free_page(current->p_pid);
        ...
        if (free_page == -1)
            ...
            // Finds empty page
            uintptr_t p_addr = free_page << PAGESHIFT;
            // Save %EAX state in local variable
            uintptr_t v_addr = current->p_registers.reg_eax;
            // Allocates PA to process
            return_value = physical_page_alloc(p_addr, current->p_pid);

            if (return_value >= 0){
                // Maps new PA to previously saved VA, with flags = user read/write
        ...
        current->p_registers.reg_eax = return_value;
        break;
    }
    ...
    /* Interruption from calling fork. current process is the parent */
    case INT_SYS_FORK: {

        // Finds a free process
        int new_process = find_free_process();
        ...
        processes[new_process].p_state = P_RUNNABLE;
        // Copies parent process's registers to child
        processes[new_process].p_registers = current->p_registers;
        // Copies parent pagetable to child
        processes[new_process].p_pagetable = copy_pagetable((x86_pagetable*) current->p_pagetable, new_process);
        
        // for each VA in parent's process ...
        for (int i = PROC_START_ADDR; i < MEMSIZE_VIRTUAL; i = i + PAGESIZE){
            vamapping map = virtual_memory_lookup(current->p_pagetable, i);

            // If page is valid
            if (map.perm & PTE_P){

                // ... and page is writable, 
                if  (map.perm & PTE_W){
                    // ... make a copy in child process
                    intptr_t new_p_addr = find_free_page_and_allocates(new_process);

                    x86_pagetable* new_pa = (x86_pagetable*) (new_p_addr << PAGESHIFT);
                    x86_pagetable* old_pa = (x86_pagetable*) (map.pn << PAGESHIFT);
                    
                    memcpy (new_pa, old_pa, PAGESIZE);
                    virtual_memory_map(processes[new_process].p_pagetable, i, new_p_addr << PAGESHIFT, PAGESIZE, PTE_P|PTE_W|PTE_U);
                } else {
                    //If page is read-only (code), just increments refcount
                    pageinfo[map.pn].refcount++;
                    virtual_memory_map(processes[new_process].p_pagetable, i, map.pa,PAGESIZE, PTE_P | PTE_W | PTE_U);
                }
            }
        }
        
        //child returns 0
        processes[new_process].p_registers.reg_eax = 0;
        //parent returns child's pid
        current->p_registers.reg_eax = new_process;
...
/*
  Function: find_free_process()
  Returns: index of new process
*/
int find_free_process(){
