...
static int program_load_segment(proc* p, const elf_program* ph,
    ...
    // If section is read-only, ...
    if ((ph->p_flags & ELF_PFLAG_READ) && !(ph->p_flags & ELF_PFLAG_WRITE)) {
        // iterates over VA's, 
        for (unsigned int va_entry = va; va_entry < end_mem; va_entry += PAGESIZE) {
            vamapping map = virtual_memory_lookup(p->p_pagetable, va_entry);
            // and re-maps PTEs as read-only
            virtual_memory_map(p->p_pagetable, va_entry, PTE_ADDR(map.pa), PAGESIZE, PTE_P | PTE_U);
        }
    }
    return 0;
}
